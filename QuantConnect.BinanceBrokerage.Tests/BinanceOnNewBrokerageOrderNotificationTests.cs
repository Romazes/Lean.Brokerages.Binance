...
namespace QuantConnect.Brokerages.Binance.Tests
{
    ...
    {
        /// <summary>
        /// Expected (action, Lean status) pair for one WS event in a lifecycle file.
        /// </summary>
        ...
        private static IEnumerable<TestCaseData> LifecycleCases
        {
            get
            {
                ...
                // Spot STOP_LOSS_LIMIT BUY: NEW (placed) => NEW (stop hit, working) => TRADE/FILLED.
                // TryGetExecution returns NewOrder for both NEW events; the dedup guard in
                // OnNewBrokerageOrder prevents double-registration at runtime.
                yield return new TestCaseData(
                ...
                // Future STOP (stop-limit) BUY: interleaved ALGO_UPDATE events are skipped by
                // TryGetExecution; the parseable ORDER_TRADE_UPDATE events are: NEW (working limit
                // order active) => TRADE/FILLED.
                yield return new TestCaseData(
                ...
                // Future STOP_MARKET BUY: ALGO_UPDATE events are skipped; the resulting MARKET
                // child order partially fills then fully fills.
                yield return new TestCaseData(
                ...
                // Future STOP_MARKET BUY canceled before triggering — all events are ALGO_UPDATE.
                // TryGetExecution returns false for every one → zero parseable events.
                yield return new TestCaseData(
                ...
                // Future LIMIT BUY canceled externally: NEW => CANCELED.
        ...
        /// <para>
        /// ALGO_UPDATE events are silently skipped (TryGetExecution returns false) — the test only
        /// asserts on the events that produce a parseable execution.
        /// </para>
        /// </summary>
        ...
        public void ExecutionLifecycleProducesExpectedActionsAndStatuses(string fileName, LifecycleStep[] expectedSteps)
        {
            ...
            // Collect only parseable events — ALGO_UPDATE events return false and are silently skipped.
            ...
            for (var i = 0; i < parseableEvents.Count; i++)
            {
                ...
                Assert.AreEqual(step.Action, action,
                    $"Event {i}: expected action {step.Action} but got {action}.");

                var leanStatus = BinanceBrokerage.ConvertOrderStatus(execution.OrderStatus);
                Assert.AreEqual(step.Status, leanStatus,
                    $"Event {i}: expected Lean status {step.Status} but got {leanStatus} " +
                    $"(raw Binance status '{execution.OrderStatus}').");
            }
        }

        /// <summary>
        /// Verifies that <see cref="BinanceBrokerage.TryGetExecution"/> routes every significant
        /// execution-type / order-type combination to the correct <see cref="ExecutionAction"/>.
        /// <list type="bullet">
        ///   <item>
        ///     <c>x:EXPIRED, o:STOP|STOP_MARKET</c> → <see cref="ExecutionAction.None"/>
        ///     (stop-trigger consumed; the child NEW arrives in a separate event).
        ///   </item>
        ///   <item>
        ///     <c>x:EXPIRED, o:&lt;other&gt;</c> → <see cref="ExecutionAction.Fill"/>
        ///     (real expiry; <c>ConvertOrderStatus</c> maps it to <see cref="OrderStatus.Invalid"/>).
        ///   </item>
        ///   <item>
        ///     <c>x:CANCELED</c> → <see cref="ExecutionAction.Cancel"/>
        ///     (routes to <c>OnFillOrder</c>, which deduplicates REST-initiated cancels).
        ///   </item>
        /// </list>
        /// </summary>
        ...
        public void TryGetExecutionRoutesToExpectedAction(
            string orderType, string execType,
            ExecutionAction expectedAction, OrderStatus expectedStatus)
        {
            // Build a minimal Spot-format executionReport payload.
            // Note: "x" = execution type, "X" = order status — they are different fields.
            // A TRADE execution leaves the order as FILLED (or PARTIALLY_FILLED), not "TRADE".
            ...
            Assert.AreEqual(expectedAction, action,
                $"Unexpected action for x={execType}, o={orderType}");

            if (expectedAction != ExecutionAction.None)
            {
                var leanStatus = BinanceBrokerage.ConvertOrderStatus(execution.OrderStatus);
                Assert.AreEqual(expectedStatus, leanStatus,
                    $"Unexpected Lean status for x={execType}, X={execution.OrderStatus}");
            }
        }
        ...
        /// attempt to register a duplicate order.
        /// </summary>
        [Test]
        public void AlreadyTrackedOrder_NewBrokerageOrderNotification_IsNotFired()
        {
            const string trackedBrokerId = "1281722417"; // taken from WebSocketGlobalSpotLimitSubmittedFilled.json

            // The first event is x:NEW for orderId=trackedBrokerId.
            // TryGetExecution returns NewOrder regardless — the dedup guard lives in OnNewBrokerageOrder.
            var firstEvent = ParseNdjson(
                Path.Combine("TestData", "WebSocketGlobalSpotLimitSubmittedFilled.json")).First();
            ...
            Assert.AreEqual(ExecutionAction.NewOrder, action,
                "TryGetExecution must return NewOrder; deduplication is OnNewBrokerageOrder's responsibility.");

            // Pre-register the broker ID so GetLeanOrder returns a non-null sentinel.
        ...
        /// <list type="bullet">
        ///   <item>
        ///     <term>REST-initiated cancel (already Canceled)</term>
        ///     <description>
        ///       <see cref="BinanceBrokerage.CancelOrder"/> fires <c>OnOrderEvent(Canceled)</c>
        ///       immediately on success.  The subsequent WS <c>x:CANCELED</c> is a duplicate and
        ///       must be dropped (0 events emitted).
        ///     </description>
        ///   </item>
        ///   <item>
        ///     <term>External cancel (Submitted → Canceled)</term>
        ///     <description>
        ///       No Lean-side cancel call was made — the order is still
        ///       <see cref="OrderStatus.Submitted"/>.  The WS event is the only notification
        ///       and must propagate (1 event with <see cref="OrderStatus.Canceled"/>).
        ///     </description>
        ///   </item>
        /// </list>
        /// </summary>
        [TestCase(OrderStatus.Canceled,  0, Description = "RestCancel_WsDuplicate_IsDropped")]
        [TestCase(OrderStatus.Submitted, 1, Description = "ExternalCancel_Propagates")]
        public void OnFillOrder_CanceledExecution_DeduplicatesRestAndPropagatesExternal(
        ...
        /// <summary>
        /// Reads a Newline-Delimited JSON (NDJSON) file — one JSON object per WS event — and yields
        /// them in document order. Blank lines between objects are handled transparently.
        /// </summary>
        ...
        private sealed class TestableBinanceBrokerage : BinanceBrokerage
        {
            ...
            private readonly Dictionary<string, OrderStatus> _trackedOrders =
                new(StringComparer.OrdinalIgnoreCase);

            /// <summary>
            /// Registers <paramref name="brokerageId"/> as an order known to Lean with a sentinel
            /// status of <see cref="OrderStatus.None"/> (caller only checks for non-null).
            /// </summary>
            public void TrackBrokerageId(string brokerageId) =>
                _trackedOrders[brokerageId] = OrderStatus.None;

            /// <summary>
            /// Registers <paramref name="brokerageId"/> with the given <paramref name="status"/> so
            /// tests can simulate any lifecycle state (e.g. already-Canceled after a REST cancel).
            /// </summary>
            public void TrackBrokerageId(string brokerageId, OrderStatus status) =>
                _trackedOrders[brokerageId] = status;
            ...
            internal override Orders.Order GetLeanOrder(string brokerageOrderId)
            {
                if (!string.IsNullOrEmpty(brokerageOrderId) &&
                    ...
                    _trackedOrders.TryGetValue(brokerageOrderId, out var status))
                {
